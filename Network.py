...
class Network(object):
    ...
    def add_node_network(self, my_new_node):
        if len(self.myNetwork) > 0:
            parent_node = self.node_mayor_avaliability()
    ...
    def find_sub_tree(self, root_node):
    ...
    def node_mayor_avaliability(self):
        target = self.mayor_availability()
        return self.search_node_avaliability(target)

    def search_node_avaliability(self, target):
